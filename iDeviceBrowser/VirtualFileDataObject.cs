    ...
    public sealed class VirtualFileDataObject : System.Runtime.InteropServices.ComTypes.IDataObject, IAsyncOperation
    {
        ...
        IEnumFORMATETC System.Runtime.InteropServices.ComTypes.IDataObject.EnumFormatEtc(DATADIR direction)
        {
            if (direction == DATADIR.DATADIR_GET)
            {
                ...
                List<FORMATETC> formatetcs = new List<FORMATETC>();
                foreach (DataObject dataObject in _dataObjects)
                {
                    formatetcs.Add(dataObject.FORMATETC);
                }
        ...
        {
            var formatCopy = format; // Cannot use ref or out parameter inside an anonymous method, lambda expression, or query expression
            if (!_dataObjects.Exists(delegate(DataObject d) { return d.FORMATETC.cfFormat == formatCopy.cfFormat; }))
            {
                return NativeMethods.DV_E_FORMATETC;
            }
            if (!_dataObjects.Exists(delegate(DataObject d) { return 0 != (d.FORMATETC.tymed & formatCopy.tymed); }))
            {
                return NativeMethods.DV_E_TYMED;
            }
            if (!_dataObjects.Exists(delegate(DataObject d) { return d.FORMATETC.dwAspect == formatCopy.dwAspect; }))
            {
        ...
        public void SetData(short dataFormat, IEnumerable<byte> data)
        {
            _dataObjects.Add(
                new DataObject
                {
                    ...
                    GetData = () =>
                    {
                        var dataArray = Enumerable.ToArray(data);
        ...
        public void SetData(IEnumerable<FileDescriptor> fileDescriptors)
        {
            ...
            bytes.AddRange(StructureBytes(new NativeMethods.FILEGROUPDESCRIPTOR { cItems = (uint)(Enumerable.Count(fileDescriptors)) }));
        ...
        private class IStreamWrapper : Stream
        {
            ...
            public override void Write(byte[] buffer, int offset, int count)
            {
                ...
                else
                {
                    ...
                    _iStream.Write(Enumerable.ToArray(Enumerable.Skip(buffer, offset)), count, IntPtr.Zero);
                }
            }
        }
